 ...
 *
 * Rendering: it keeps a precomputed "coloured hair layer" ([coloredLayer]) built by
 * [HairRecolor.colorizeInto] from the current [HairColorStyle] (solid or gradient
 * ombré) + [shine], and draws it over the photo at the chosen [intensity] (a
 * draw-time alpha). The layer is recomputed lazily (only when the style, shine or
 * mask change) so colour/shine edits and brush strokes stay responsive.
 *
 * In brush mode the user paints (adds hair to the mask) or erases. Strokes are
 * recorded so undo / redo / reset rebuild the mask from the segmentation base.
 */
class HairCanvasView @JvmOverloads constructor(
    ...
    // --- recolour state ---
    private var style: HairColorStyle = HairColorStyle.DEFAULT
    private var intensity: Int = 255
    private var shine: Int = HairRecolor.DEFAULT_SHINE
    private var coloredLayer: Bitmap? = null
    private var coloredDirty = true
    private var gradTop = 0f
    private var gradBottom = 0f
    ...
    fun setImage(originalBitmap: Bitmap, mask: Bitmap) {
        ...
        strokes.clear()
        redoStrokes.clear()
        // Span the gradient over the actual hair extent for a proper ombré.
        val bounds = HairRecolor.verticalMaskBounds(mask)
        gradTop = bounds?.first ?: 0f
        gradBottom = bounds?.second ?: mask.height.toFloat()
        coloredDirty = true
        resetDisplayMatrix()
        invalidate()
    }

    fun setStyle(newStyle: HairColorStyle) {
        style = newStyle
        coloredDirty = true
        invalidate()
    }

    fun setShine(value: Int) {
        shine = value.coerceIn(0, 255)
        coloredDirty = true
        invalidate()
    }

    /** Opacity only — applied as a draw-time alpha, no layer recompute needed. */
    ...
    fun exportBitmap(): Bitmap? {
        ...
        return HairRecolor.compose(src, mask, style, intensity, shine, gradTop, gradBottom)
    }
    ...
    private fun ensureColoredLayer() {
        val mask = workingMask ?: return
        if (!coloredDirty && coloredLayer != null) return
        var layer = coloredLayer
        if (layer == null || layer.width != mask.width || layer.height != mask.height) {
            layer?.recycle()
            layer = Bitmap.createBitmap(mask.width, mask.height, Bitmap.Config.ARGB_8888)
            coloredLayer = layer
        }
        HairRecolor.colorizeInto(layer, mask, style, shine, original, gradTop, gradBottom)
        coloredDirty = false
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (!compare) {
            ensureColoredLayer()
            val layer = coloredLayer
            if (layer != null) {
                recolorPaint.reset()
                recolorPaint.isAntiAlias = true
                recolorPaint.isFilterBitmap = true
                recolorPaint.alpha = intensity
                val sr = Rect(0, 0, layer.width, layer.height)
                val dr = Rect(0, 0, src.width, src.height)
                canvas.drawBitmap(layer, sr, dr, recolorPaint)
            }
        }
        canvas.restore()

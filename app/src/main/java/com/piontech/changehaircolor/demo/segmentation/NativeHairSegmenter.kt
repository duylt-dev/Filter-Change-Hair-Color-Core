package com.piontech.changehaircolor.demo.segmentation
 ...
 */
class NativeHairSegmenter private constructor(private val engine: DevHairSegmentation) {
    ...
    fun segment(bitmap: Bitmap): Bitmap? {
        ...
        val mask = Bitmap.createBitmap(info.image_width, info.image_height, Bitmap.Config.ARGB_8888)
    ...
    companion object {
        ...
        private fun ensureEven(src: Bitmap): Bitmap {
            var b = src
            if (b.width % 2 != 0) b = Bitmap.createScaledBitmap(b, b.width + 1, b.height, false)
            if (b.height % 2 != 0) b = Bitmap.createScaledBitmap(b, b.width, b.height + 1, false)
            return b
        }
        ...
        private fun encodeYUV420SP(yuv420sp: ByteArray, argb: IntArray, width: Int, height: Int) {
            ...
            for (j in 0 until height) {
                for (i in 0 until width) {
                    ...
                    if (j % 2 == 0 && index % 2 == 0) {
                        yuv420sp[uvIndex++] = (if (v < 0) 0 else min(v, 255)).toByte()
